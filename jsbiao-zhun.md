...
### IIFE（立即执行的函数表达式） {#iife（立即执行的函数表达式）}
...
### 理解 JavaScript 的定义域和定义域提升 {#理解-javascript-的定义域和定义域提升}
...
### 使用 ECMA Script 5 {#使用-ecma-script-5}

建议使用 ECMA Script 5 中新增的语法糖和函数。这将简化你的程序，并让你的代码更加灵活和可复用。

---

### 数组和对象的属性迭代 {#数组和对象的属性迭代}
...
### 自定义 toString\(\) 方法 {#自定义-tostring()-方法}
...
### 三元条件判断（if 的快捷方法） {#三元条件判断（if-的快捷方法）}
